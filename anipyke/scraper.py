...
class WebScraper(object):
                ...
                for attr_name, attr_follow in attrs:
                    if attr_name in el.attrs:
                        ...
                        if (len(link) > 0) and ((protocol_in_front.match(link) is None) or link.startswith("http")):
                            def modifier(url):
                                logger.debug(f"replacing {link} with {url}")
    ...
    def _scrape_immediate(self, url, follow_from, pbar, show_progress):
        if url in self.urls_scraped:
            return self.urls_scraped[url]
        result = self._scrape_immediate2(url, follow_from, pbar, show_progress)
        self.urls_scraped[url] = result
        return result

    def _scrape_immediate2(self, url, follow_from, pbar, show_progress):
        last_url_queue_len = len(self.urls_queue)

        if url.startswith("http://members.tripod.com/bin/counter/"):
            return False
        elif url.startswith("http://leader.linkexchange.com"):
            return False
        while "//" in url[9:]:
        ...
        if os.path.exists(raw_cache_path):
            ...
            if success:
                ...
                if self.is_targetted_location(url) or (not follow_from):
                    data = db.get_http_contents_from_wayback(url, wayback_date, ignore_html=(not follow_from))
                    if data is not None:
        ...
        if is_html:
            ...
            for follow_to_link, follow_should, followed_link_modify in self.find_all_links(html):
                ...
                if follow_from and ((not follow_should) or self.is_targetted_location(follow_to_queue_link)):
                    ...
                    if self.is_targetted_location(follow_to_queue_link):
                        ...
                        success = self._scrape_immediate(follow_to_queue_link, url_follow, pbar, show_progress)
                        pbar.total += 1

                    if success or follow_to_link.startswith("http://") or follow_to_link.startswith("https://"):
            ...
            curr_url_queue_len = len(self.urls_queue)
            pbar.total += curr_url_queue_len - last_url_queue_len
        else:
            # binary file
            create_dir_parent(target_file_path)
    ...
    def scrape2(self, show_progress):
        ...
        while len(self.urls_queue) > 0:
            ...
            self._scrape_immediate(url, follow_from, pbar, show_progress)

    def scrape(self, show_progress=True):
        self.scrape2(show_progress)
